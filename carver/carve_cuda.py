...
class Core:
    ...
    def _get_keep(self, min_idx: int, backtrack: np.ndarray) -> np.ndarray:
        r, _ = backtrack.shape
        keep = np.ones_like(backtrack, dtype=bool)
        j = min_idx
...
# import image and carve it 
def main():

    # load image
    img = cv2.imread("/home/siddharth/vscode/classwork/SIP/fluidimageresizing/Image/tower.jpg")
    ...
    core = Core(gpu_id)
    start = time.time()
    img, mask_remove, mask_protect = Core.carve_column(img, None, None)
    ...
    plt.subplot(122)
    plt.imshow(mask_remove.astype(np.uint8))
    plt.show()
